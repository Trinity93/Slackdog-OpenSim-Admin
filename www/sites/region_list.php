...
if(isset($_GET['AStart'])){$AStart=$_GET['AStart'];};
if(isset($_GET['ALimit'])){$ALimit=$_GET['ALimit'];};

if(!isset($AStart)) $AStart = $AnzeigeStart;
if(!isset($ALimit)) $ALimit = $AnzeigeLimit;
...
<!--//END LIMIT AND SEARCH ROW -->	  </TD>
    ...
    <TR <? if($w==2){$w=0; echo"style='BACKGROUND-COLOR: #e8e0c5'";}else{echo"style='BACKGROUND-COLOR: #e8eff5'";}?>>
      ...
      <TD><DIV style="COLOR: #9966ff"><A style="cursor:pointer" onClick="window.open('app/region/?x=<?=$locX?>&y=<?=$locY?>','mywindow','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,copyhistory=no,width=800,height=400')"><B><u>Click for more Info</u></B></A></DIV></TD>
      <TD bgcolor="#FFFFFF">&nbsp;</TD>
	</TR>
	<?
	}
