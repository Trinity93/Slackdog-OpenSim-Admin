...
/* ---------------------------------------------------------------------- *\
  ...
  Description : default configuration settings for digi editor
\* ---------------------------------------------------------------------- */
...
this.width =  "auto";
this.height = "auto";
this.bodyStyle = 'background-color: transparent; font-family: "Verdana"; font-size: x-small;';
this.imgURL = _editor_url + 'editor/images/';
this.debug  = 0;
...
//    { name: "Arial Ror",    className: "headline2", classStyle: "font-family: arial black, arial; font-size: 12px; letter-spacing: -2px; color:red" },
...
this.btnList = {
    ...
    "underline":      ['Underline',            'Unterstrichen',          'editor_action(this.id)',  'ed_format_underline.gif'],
    ...
    "unorderedlist":  ['InsertUnorderedList',  'Aufzählungszeichen',      'editor_action(this.id)',  'ed_list_bullet.gif'],
    ...
    "forecolor":      ['ForeColor',            'Schrift Farbe',         'editor_action(this.id)',  'ed_color_fg.gif'],
    "backcolor":      ['BackColor',            'Hintergrund Farbe',   'editor_action(this.id)',  'ed_color_bg.gif'],
    "horizontalrule": ['InsertHorizontalRule', 'Linie zeichen',    'editor_action(this.id)',  'ed_hr.gif'],
    "createlink":     ['CreateLink',           'Web Link einfügen',    'editor_action(this.id)',  'ed_link.gif'],
    "insertimage":    ['InsertImage',          'Bild einfügen',       'editor_action(this.id)',  'ed_image.gif'],
    "inserttable":    ['InsertTable',          'Insert Table',       'editor_action(this.id)',  'insert_table.gif'],
    "htmlmode":       ['HtmlMode',             'HTML Source ansehen',   'editor_setmode(\''+objname+'\')', 'ed_html.gif'],
    "popupeditor":    ['popupeditor',          'Editor vergrößern',     'editor_action(this.id)',  'fullscreen_maximize.gif'],
   ...
   // end: custom buttons
    ...
    "help":           ['showhelp',             'Hilfemenü',  'editor_action(this.id)',  'ed_help.gif']};
...
function editor_generate(objname,userConfig) {
  ...
  if (!config.height || config.height == "auto") {
    ...
    else                       { config.height = '100%'; }              // default
  }
...
function editor_action(button_id) {
  ...
  // help popup
  if (cmdID == 'showhelp') {
    window.open(_editor_url + "editor/popups/editor_help.html", 'EditorHelp');
    return;
  }

  // popup editor
  if (cmdID == 'popupeditor') {
    window.open(_editor_url + "editor/popups/fullscreen.html?"+objname,
                'FullScreen',
                'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=750,height=550');
    return;
  }
  ...
  // Custom2
    ...
    var myText = showModalDialog(_editor_url + "editor/popups/custom2.html",
  ...
  // fgColor and bgColor
    ...
    var newcolor = showModalDialog(_editor_url + "editor/popups/select_color.html", oldcolor, "resizable: no; help: no; status: no; scroll: no;");
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
      ...
      showModalDialog(_editor_url + "editor/popups/insert_image.html", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }

    // insert table
    else if (cmdID.toLowerCase() == 'inserttable'){
      showModalDialog(_editor_url + "editor/popups/insert_table.html?"+objname,
                                 window,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
    }
...
function editor_setmode(objname, mode) {
 ...
 // src="' +_editor_url+ 'editor/popups/blank.html"
...
function editor_about(objname) {
  showModalDialog(_editor_url + "editor/popups/about.html", window, "resizable: yes; help: no; status: no; scroll: no; ");
}
